...
class Equations
{
    double x0, x1, t0, t1, v1, C1, C2, C3, C4;

    public void Init(double x0, double x1, double t0, double t1, double C1, double C2, double C3, double C4)
    {
        this.x0 = x0;
        this.x1 = x1;
        this.t0 = t0;
    ...
    {
        return C1*(t*t - t0*t0)*(t-t0)*(1 - 0)*(x-x0)  -  C2*(t-t0)*(t-t0)  -  x;
    }

    // used to calculate v(t) and v1 in first phase
    public double P1v(double t, double x)
    {
        return C1*(t*t - t0*t0)*(1 - 0)  -  C2*(t-t0);
    }
    ...
    public double P2t(double t, double x)
    {
        return C3*(1 - 0)*(x-x1)*(t-t1)*(t-t1)  -  C2*(t-t1)*(t-t1)  +  x1  -  x;
    }

    // used to calculate v(t) in second phase
    public double P2v(double t, double x)
    {
        return C3*(1 - 0)*(t-t1)*(t-t1) - C2*(t-t1) + v1;
    }
